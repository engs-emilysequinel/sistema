# 📌 SPRINT -> APLICATIVO DE RPG
## 📅 PERÍODO DA SPRINT:  01/03/2026 -> 27/03/26

## 👥EQUIPE:
### 👤PRODUCT OWNER - MARYA
### 👤SCRUM MASTER - EMILY
### 👥DESENVOLVEDORAS - BIANCA, ISABELY, EDUARDA

# 🎯OBJETIVO DA SPRINT: 
### Desenvolver e entregar um aplicativo funcional de um sistema de RPG digital, onde o sistema deve permitir que usuários se cadastrem, criem e utilizem fichas de personagens de forma iterativa. O foco principal da SPRINT será implementar as principais funcionalidades para suportar uma sessão de RPG e a interação entre jogadores e mestre de forma clara.

#### 🔴TAREFAS DE ALTA PRIORIDADE
#### 🟡TAREFAS DE MEDIA PRIORIDADE
#### 🟢TAREFAS DE BAIXA PRIORIDADE


## TAREFAS FEITAS
#### 🔴Ler os livros de RPG  👩‍💻 **Todos**
#### 🔴Baixar os livros de RPG e subir para o Software 👩‍💻  **Todos**
#### 🔴Subir as fichas de rpg 👩‍💻 **Isabely, Bianca, Eduarda**
#### 🔴Desenvolver interface 100% responsiva (mobile + tablet + desktop) 👩‍💻 **Eduarda**
#### 🔴Interface do app 👩‍💻 **Eduarda**
#### 🟡Software do tabletop 👩‍💻 **Isabely**
#### 🔴Página de login 👩‍💻 **Bianca**
#### 🔴Adicionar login social (Google, Apple, Discord) 👩‍💻 **Bianca**
#### 🔴Implementar recuperação de senha 👩‍💻 **Bianca**
#### 🔴Criar tutorial interativo para novos usuários 👩‍💻 **Isabely**
#### 🔴 Criar perfis separados (Jogador x Mestre) com dashboard inicial personalizado 👩‍💻 **Bianca**
#### 🔴Adicionar validação automática de regras (pontos de atributos, limites de classe, etc.) 👩‍💻 **Eduarda**
#### 🔴Implementar sistema de níveis e experiência automática entre fichas e campanha 👩‍💻 **Isabely**

 # FAZENDO:
 #### 🔴Sistema de criação de ficha 👩‍💻 **Bianca**
 #### 🔴Sincronização em tempo real de todas as fichas e iniciativa 👩‍💻 **Eduarda**
 #### 🔴Implementar exportação da ficha para PDF/impressão 👩‍💻 **Bianca**
 #### 🟡Criar um espaço para os livros dos jogadores, atualizados 👩‍💻 **Eduarda**
 #### 🔴Deixar as fichas de rpg interativas para ser preechida (ficha digital) 👩‍💻 **Isabely**
 #### 🟡Permitir upload de imagem própria 👩‍💻 **Isabely**
 #### 🟡(aparência do personagem) um espaço para os jogadores escolherem se querem desenhar ou utilizar os modelos prontos). 👩‍💻 **Isabely**
 #### 🔴Nuvem para guardar imagens enviadas e outros arquivos 👩‍💻 **Eduarda**
 #### 🟡Aba para salvar a historia 👩‍💻 **Bianca**
 #### 🟡Opção de convidar jogadores 👩‍💻 **Eduarda**

 # A FAZER:
 #### 🔴Criar uma aba para cada raça 👩‍💻 **Isabely**
 #### 🟡Desenvolver visualizador simples de mapa/combate (grid 2D) 👩‍💻 **Eduarda**
 #### 🟡Criar uma aba para cada classe 👩‍💻 **Isabely**
 #### 🟡Criar um espaço para visualizar os resumos de cada classe 👩‍💻 **Bianca**
 #### 🟡Dicas de combinação entre raça, antecedente e classe 👩‍💻 **Bianca**
 #### 🟡Um escudo do mestre 👩‍💻 **Eduarda**
 #### 🟡Um bestiario 👩‍💻 **Isabely**
 #### 🟢Uma area de criação de NPC, aleatorização de caracteristica pra criar mais rapido 👩‍💻 **Eduarda**
 #### 🟢Criar sistema de convites por link/código para campanhas 👩‍💻 **Bianca**
 #### 🟢Criar uma campanha de rpg (onde vai ter todas as coisas acima) 👩‍💻 **Isabely**
 #### 🟢Adicionar a play store e apple 👩‍💻 **Todas**